...
from bugs.forms import BugForm, BugCommentForm
...
def view_issue(request,project_id,bug_id):
    ...
    context = RequestContext(request, {})
    context['project'] = get_object_or_404(Project,pk=project_id)
    current_bug = get_object_or_404(Bug,pk=bug_id)
    created = False
    ...
    comments = BugComment.objects.filter(bug=current_bug)
    context['comments'] = sorted(comments, key=operator.attrgetter('created_date'))
    context['bug'] = current_bug
    context['bug_form'] = form
    context['created'] = created
    return HttpResponse(template.render(context))
